...
def write_differ_information(all_differ_dict,exist_question,auto_check_database_dict,word_DMP_database_dict,write_differ_path="E:\\DMP_database\\differ_information.xls"):
    ...
    for both_exist_table in range(len(all_differ_dict["both_exist_database_table_differ"]['dw_ods']["table_differ"]["both_exist_table"])):
        ...
        createVar['sheet' + str(both_exist_table)].write(0, 10, "hive表中分区字段")
        createVar['sheet' + str(both_exist_table)].write(0, 11, "分区字段类型")

        partition_number=0
        for table_partiton in auto_check_database_dict["database_name"]['dw_ods']["table_name"][both_exist_table_name]["field_partition_information"]:
            partition_number+=1
            createVar['sheet' + str(both_exist_table)].write(partition_number, 10, table_partiton)
            createVar['sheet' + str(both_exist_table)].write(partition_number, 11, auto_check_database_dict["database_name"]['dw_ods']["table_name"][both_exist_table_name]["field_partition_information"][table_partiton])
